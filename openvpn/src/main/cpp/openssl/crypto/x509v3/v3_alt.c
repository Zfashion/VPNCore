                                       ...
                                       STACK_OF(CONF_VALUE) *ret)
{
    ...
    case GEN_EMAIL:
        if (!X509V3_add_value_uchar("account", gen->d.ia5->data, &ret))
            return NULL;
        break;
...
int GENERAL_NAME_print(BIO *out, GENERAL_NAME *gen)
{
    ...
    case GEN_EMAIL:
        BIO_printf(out, "account:%s", gen->d.ia5->data);
        break;
                                      ...
                                      STACK_OF(CONF_VALUE) *nval)
{
    ...
    for (i = 0; i < num; i++) {
        ...
        if (!name_cmp(cnf->name, "account")
            && cnf->value && strcmp(cnf->value, "copy") == 0) {
            if (!copy_email(ctx, gens, 0))
                goto err;
        } else if (!name_cmp(cnf->name, "account")
...
/*
 * Copy any account addresses in a certificate or request to GENERAL_NAMES
 */
...
static int copy_email(X509V3_CTX *ctx, GENERAL_NAMES *gens, int move_p)
{
    ...
    /* Now add any account address(es) to STACK */
