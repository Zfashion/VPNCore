...
void Java_com_blinkt_openvpn_core_NativeUtils_jniclose(JNIEnv *env,jclass jo, jint fd)
{
	int ret = close(fd);
}
...
// Java_de_blinkt_openvpn_core_NativeUtils_getJNIAPI to Java_com_blinkt_openvpn_core_NativeUtils_getJNIAPI  tanml
jstring Java_com_blinkt_openvpn_core_NativeUtils_getJNIAPI(JNIEnv *env, jclass jo)
{

    return (*env)->NewStringUTF(env, TARGET_ARCH_ABI);
}

jstring Java_com_blinkt_openvpn_core_NativeUtils_getOpenVPN2GitVersion(JNIEnv *env, jclass jo)
{
