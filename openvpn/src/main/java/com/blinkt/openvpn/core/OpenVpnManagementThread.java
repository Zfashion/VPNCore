...
public class OpenVpnManagementThread implements Runnable, OpenVPNManagement {
    ...
    private final HandlerThread mResumeHandlerThread;
    private LocalSocket mSocket;
    private VpnProfile mProfile;
    private OpenVPNImpl mOpenVPNImpl;
    ...
    public OpenVpnManagementThread(VpnProfile profile, OpenVPNImpl openVpnService) {
        mProfile = profile;
        mOpenVPNImpl = openVpnService;
        mResumeHandlerThread = new HandlerThread("");
        mResumeHandlerThread.start();
        mResumeHandler = new Handler(mResumeHandlerThread.getLooper());
    }
    ...
    private void protectFileDescriptor(FileDescriptor fd) {
        try {
            ...
            boolean result = mOpenVPNImpl.protect(fdint);
            if (!result)
    ...
    private void processInfoMessage(String info) {
        if (info.startsWith("OPEN_URL:")) {
            mOpenVPNImpl.trigger_url_open(info);
        }
    }
    ...
    private void processNeedCommand(String argument) {
        ...
        switch (needed) {
            ...
            case "DNSSERVER":
            case "DNS6SERVER":
                mOpenVPNImpl.addDNS(extra);
                break;
            case "DNSDOMAIN":
                mOpenVPNImpl.setDomain(extra);
                break;
            case "ROUTE": {
            ...
            */
                ...
                if (routeparts.length == 5) {
                    //if (com.sino.app.anyvpn.utils.AppLog.isDebug)
                    //                assertEquals("dev", routeparts[3]);
                    mOpenVPNImpl.addRoute(routeparts[0], routeparts[1], routeparts[2], routeparts[4]);
                } else if (routeparts.length >= 3) {
                    mOpenVPNImpl.addRoute(routeparts[0], routeparts[1], routeparts[2], null);
                } else {
            ...
            case "ROUTE6": {
                String[] routeparts = extra.split(" ");
                mOpenVPNImpl.addRoutev6(routeparts[0], routeparts[1]);
                break;
            }
            case "IFCONFIG":
                String[] ifconfigparts = extra.split(" ");
                int mtu = Integer.parseInt(ifconfigparts[2]);
                mOpenVPNImpl.setLocalIP(ifconfigparts[0], ifconfigparts[1], mtu, ifconfigparts[3]);
                break;
            case "IFCONFIG6":
                String[] ifconfig6parts = extra.split(" ");
                mtu = Integer.parseInt(ifconfig6parts[1]);
                mOpenVPNImpl.setMtu(mtu);
                mOpenVPNImpl.setLocalIPv6(ifconfig6parts[0]);
                break;
            case "PERSIST_TUN_ACTION":
                // check if tun cfg stayed the same
                status = mOpenVPNImpl.getTunReopenStatus();
                break;
            case "OPENTUN":
    ...
    private boolean sendTunFD(String needed, String extra) {
        ...
        ParcelFileDescriptor pfd = mOpenVPNImpl.openTun();
        if (pfd == null)
            return false;
    ...
    private void processPWCommand(String argument) {
        ...
        if (pw != null) {
            ...
            mOpenVPNImpl.requestInputFromUser(R.string.password, needed);
    ...
    private void processSignCommand(String argument) {
        ...
        String signed_string = mProfile.getSignedData(mOpenVPNImpl.getContext(), arguments[0], pkcs1padding);
    ...
    public boolean stopVPN(boolean replaceConnection) {
        ...
        if (stopSucceed) {
            mShuttingDown = true;
            if (mResumeHandlerThread != null) {
                if (mResumeHandlerThread.isAlive()){
                    VPNLog.d("OpenVpnManagementThread->mResumeHandlerThread.quitSafely();");
                    mResumeHandlerThread.quitSafely();
                }
            }
        }
        return stopSucceed;
