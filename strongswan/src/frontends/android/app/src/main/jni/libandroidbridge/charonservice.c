							   ...
							   char *appdir, jboolean byod)
{
	...
	//context用于初始化nNetworkManager
    jclass serviceClass = (*env)->GetObjectClass(env,service);
    jmethodID getContextMethodID = (*env)->GetMethodID(env,serviceClass , (char *) "getContext",(char *) "()Landroid/content/Context;");
    jobject context = (*env)->CallObjectMethod(env,service, getContextMethodID);
	...
	INIT(this,
		...
		.network_manager = network_manager_create(context),
 ...
 */
JNI_METHOD(Ikev2VPNImpl, initializeCharon, jboolean,
 ...
 */
JNI_METHOD(Ikev2VPNImpl, deinitializeCharon, void)
{
 ...
 * Initiate SA
 */
JNI_METHOD(Ikev2VPNImpl, initiate, void,
	jstring jconfig)
{
	settings_t *settings;
	char *config;
